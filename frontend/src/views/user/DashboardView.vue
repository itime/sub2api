    ...
    <div class="space-y-6">
      ...
      <template v-else-if="stats">
        <UserDashboardStats
          ...
          :system-token-accounts="systemTokenAccounts"
...
const systemTokenAccounts = ref<{ active: number; rateLimit: number } | null>(null)
...
const loadStats = async () => {
  loading.value = true
  systemTokenAccounts.value = null
  try {
    await authStore.refreshUser()
    stats.value = await usageAPI.getDashboardStats()
    if (authStore.isAdmin) {
      const adminStats = await adminAPI.dashboard.getStats()
      systemTokenAccounts.value = {
        active: adminStats.today_active_accounts ?? 0,
        rateLimit: adminStats.today_ratelimit_accounts ?? 0
      }
    }
  } catch (error) {
