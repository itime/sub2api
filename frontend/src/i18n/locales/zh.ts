export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      dataImportResultTitle: '导入结果',
      dataImportHint: '上传导出的 JSON 文件以批量导入账号与代理。支持完整备份包、单账号对象，或多文件批量导入。',
      dataImportWarning: '导入将创建新账号与代理，或更新相同平台、类型、邮箱的已有账号；请确认已有数据不会冲突。',
      dataImportFile: '数据文件',
      dataImportFileHint: '支持 .json、CPA 单文件、.zip/.gz 压缩包，可多选',
      dataImportSelectedFiles: '已选择 {count} 个文件',
      dataImportPresetProxy: '默认代理',
      dataImportPresetProxyHint: 'JSON 中未指定 proxy_key 的账号将绑定此代理。',
      dataImportPresetGroupsHint: 'JSON 中未指定 group_ids 的账号将加入所选分组。',
      dataImportButton: '开始导入',
      dataImporting: '导入中...',
      dataImportSelectFile: '请选择数据文件',
      dataImportParseFailed: '数据解析失败，请确认文件为有效 JSON',
      dataImportParseSummary: '已解析 {files} 个文件：代理 {proxies} 条，账号 {accounts} 条',
      dataImportParseSkippedSummary: '解析跳过 {skipped} 条（缺少必填字段）',
      dataImportBatchProgress: '正在提交第 {current}/{total} 批（本批 {accounts} 条账号）',
      dataImportBatchDone: '已完成 {done}/{total} 批',
      dataImportBatchCreated: '累计入库 {created} 条，失败 {failed} 条',
      dataImportProgressTitle: '正在导入',
      dataImportProgressParsing: '正在并行解析文件…',
      dataImportProgressUploading: '正在并发入库…',
      dataImportProgressParseDetail: '已解析 {current}/{total} 个文件',
      dataImportProgressUploadDetail: '已提交 {current}/{total} 批',
      dataImportProgressArchive: '正在上传并解压压缩包…',
      dataImportProgressArchiveDetail: '压缩包 {current}/{total}（上传 {percent}%）',
      dataImportArchiveQueued: '待处理压缩包 {count} 个',
      dataImportArchiveOnly: '将导入 {count} 个压缩包（自动解压并识别 CPA 格式）',
      dataImportArchiveDone: '{name}：入库 {created}，跳过 {skipped}，失败 {failed}',
      dataImportFastImport: '快速导入（先入库）',
      dataImportFastImportHint: '跳过邮箱重复检测，直接写入数据库，显著加快大批量导入。可用「测试当前筛选」稍后检测可用性。',
      dataImportTestAfter: '导入完成后测试连接',
      dataImportTestAfterHint: '关闭导入弹窗后，在后台并发测试新导入账号并自动更新状态。',
      dataImportRememberConfig: '下次导入按这次配置',
      dataImportRememberConfigHint: '勾选后保存代理、分组、快速导入和导入后测试等选项；下次打开只需选择文件。不勾选则每次重新选择。',
      dataImportTestingAfter: '已入库 {count} 个账号，正在后台测试连接…',
      dataImportMissingFields: '缺少字段：{fields}',
      dataImportFileUnrecognized: '无法识别账号结构，需包含 accounts 数组或单账号字段（credentials/access_token 等）',
      dataImportInvalidJsonRoot: 'JSON 根节点无效，需为对象或数组',
      dataImportResultParseFailed: '解析未通过，未提交导入',
      dataImportParseSkipMissing: '缺少字段：{fields}',
      dataImportParseSkipUnrecognized: '无法识别账号结构',
      dataImportParseSkipFileUnrecognized:
        '文件结构无法识别（字段：{keys}）。至少需要 {fields}，并建议包含 {nameField} 以便生成账号名。',
      dataImportParseEmptyObject: '空对象',
      dataImportFieldObject: '有效的 JSON 对象',
      dataImportFieldCredentials: 'credentials / access_token',
      dataImportFieldName: 'name / email',
      dataImportFailed: '数据导入失败',
      dataImportTimeout: '请求超时（已等待 {minutes} 分钟）。压缩包较大或数据库较慢时请耐心等待，或改为逐个上传较小的压缩包。',
      dataImportNetworkFailed: '网络连接失败：{detail}。请确认服务仍在运行、压缩包未超过 256MB，并避免同时上传多个大文件。',
      dataImportHttpFailed: '服务器返回 HTTP {status}，请查看后端日志获取详情。',
      dataImportResult: '导入结果',
      dataImportResultSummary: '提交代理 {proxies_received} 条、账号 {accounts_received} 条。代理：创建 {proxy_created}，复用 {proxy_reused}，跳过 {proxy_skipped}，失败 {proxy_failed}。账号：创建 {account_created}，更新 {account_updated}，跳过 {account_skipped}，失败 {account_failed}。',
      dataImportResultSuccessTitle: '导入完成',
      dataImportResultPartial: '导入部分成功',
      dataImportResultFailed: '导入失败',
      dataImportResultEmpty: '未产生任何导入结果',
      dataImportNoOutcomeHint:
        '未创建、更新或失败任何记录。请确认 JSON 为 sub2api 备份格式（含 accounts 数组），或 Codex session / openai-free 单账号格式（含 access_token 或 credentials）。',
      dataImportActionCreated: '成功创建',
      dataImportActionUpdated: '成功更新',
      dataImportActionReused: '已复用',
      dataImportActionSkipped: '已跳过',
      dataImportActionFailed: '失败',
      dataImportKindProxy: '代理',
      dataImportKindAccount: '账号',
      ...
      bulkActions: {
        ...
        testConnectionFiltered: '测试当前筛选',
        testingConnection: '测试中...',
        testConnectionConfirm: '将对选中的 {count} 个账号并发测试连接，并根据上游响应自动更新账号状态。继续吗？',
        testConnectionFilteredConfirm: '将对当前筛选条件下的 {count} 个账号并发测试连接，并根据上游响应自动更新账号状态。继续吗？',
        testConnectionSuccess: '测试完成：{count} 个账号可用',
        testConnectionPartial: '测试完成：可用 {success} 个，失败 {failed} 个（已更新 {updated} 个账号状态）',
        noSelection: '请先选择要测试的账号',
        resetStatusSuccess: '已成功重置 {count} 个账号状态',
        refreshTokenSuccess: '已成功刷新 {count} 个账号令牌',
        partialSuccess: '操作部分完成：{success} 成功，{failed} 失败'
      },
      bulkTestAll: '批量连通测试',
      bulkTestAllHint: '按状态批量测试当前筛选条件下的全部账号，测试完成后自动刷新列表',
      bulkTest: {
        title: '批量测试连接',
        scopeSelected: '已选择 {count} 个账号',
        scopeFiltered: '当前筛选条件下的全部账号',
        scopeStatuses: '按状态测试全部账号',
        statusFilterLabel: '选择要测试的账号状态',
        statusFilterHint: '将测试当前分组、平台、搜索等筛选条件下，所选状态的全部账号（不限于当前页）',
        selectAllStatuses: '全选状态',
        clearStatuses: '清除',
        noStatusSelected: '请至少选择一种账号状态',
        maxAccountsWarning: '单次最多测试 {max} 个账号，当前已选 {count} 个。请减少状态范围或缩小筛选条件。',
        scopeHint: '将根据上游响应自动更新账号状态',
        concurrencyHint: '每批最多 {workers} 个并发探测，多批并行以加速测试',
        accountCount: '待测试账号：{count} 个',
        collectingAccounts: '正在收集待测账号列表…',
        running: '正在批量测试连接...',
        stopAndRefresh: '停止并刷新列表',
        stopping: '正在停止…',
        refreshList: '刷新列表',
        resultStopped: '测试已停止',
        resultStoppedSummary: '已完成 {done}/{total} 个；已更新 {updated} 个账号状态，列表已刷新',
        progressDetail: '已完成 {done} / {total}',
        resultSuccess: '批量测试完成',
        resultPartial: '批量测试完成（部分失败）',
        resultSummary: '可用 {success} 个，失败 {failed} 个，已更新 {updated} 个账号状态',
        stats: {
          success: '成功',
          failed: '失败',
          deactivated: '停用',
