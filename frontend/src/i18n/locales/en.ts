export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      dataImportResultTitle: 'Import Result',
      dataImportHint: 'Upload exported JSON files to import accounts and proxies. Supports full backup payloads, single-account objects, and multi-file batches.',
      ...
      dataImportFileHint: 'Supports .json, CPA single files, and .zip/.gz archives',
      dataImportSelectedFiles: '{count} files selected',
      dataImportPresetProxy: 'Default proxy',
      dataImportPresetProxyHint: 'Accounts without proxy_key in JSON will use this proxy.',
      dataImportPresetGroupsHint: 'Accounts without group_ids in JSON will join the selected groups.',
      dataImportButton: 'Start Import',
      dataImporting: 'Importing...',
      dataImportSelectFile: 'Please select a data file',
      dataImportParseFailed: 'Failed to parse data file. Ensure the file is valid JSON.',
      dataImportParseSummary: 'Parsed {files} file(s): {proxies} proxy record(s), {accounts} account record(s)',
      dataImportParseSkippedSummary: 'Skipped {skipped} record(s) during parsing (missing required fields)',
      dataImportBatchProgress: 'Submitting batch {current}/{total} ({accounts} account(s) in this batch)',
      dataImportBatchDone: 'Completed {done}/{total} batch(es)',
      dataImportBatchCreated: 'Saved {created} account(s), {failed} failed',
      dataImportProgressTitle: 'Importing',
      dataImportProgressParsing: 'Parsing files in parallel…',
      dataImportProgressUploading: 'Saving accounts concurrently…',
      dataImportProgressParseDetail: 'Parsed {current}/{total} file(s)',
      dataImportProgressUploadDetail: 'Submitted {current}/{total} batch(es)',
      dataImportProgressArchive: 'Uploading and extracting archives…',
      dataImportProgressArchiveDetail: 'Archive {current}/{total} (upload {percent}%)',
      dataImportArchiveQueued: '{count} archive(s) queued',
      dataImportArchiveOnly: 'Importing {count} archive(s) with auto CPA detection',
      dataImportArchiveDone: '{name}: saved {created}, skipped {skipped}, failed {failed}',
      dataImportFastImport: 'Fast import (save first)',
      dataImportFastImportHint: 'Skip email deduplication and write directly to the database for much faster bulk imports. Use "Test Filtered" later to check availability.',
      dataImportTestAfter: 'Test connection after import',
      dataImportTestAfterHint: 'After closing this dialog, run concurrent connectivity tests on newly imported accounts and auto-update status.',
      dataImportRememberConfig: 'Use this configuration next time',
      dataImportRememberConfigHint: 'When checked, saves proxy, groups, fast import, and post-import test options. Next time you only need to pick files. Unchecked means nothing is saved.',
      dataImportTestingAfter: 'Saved {count} account(s). Running background connectivity tests…',
      dataImportMissingFields: 'Missing fields: {fields}',
      dataImportFileUnrecognized: 'Unrecognized account structure; expected an accounts array or single-account fields (credentials/access_token, etc.)',
      dataImportInvalidJsonRoot: 'Invalid JSON root; expected an object or array',
      dataImportResultParseFailed: 'Parsing failed; nothing was submitted',
      dataImportParseSkipMissing: 'Missing fields: {fields}',
      dataImportParseSkipUnrecognized: 'Unrecognized account structure',
      dataImportParseSkipFileUnrecognized:
        'File structure is unrecognized (fields: {keys}). At least {fields} is required, and {nameField} is recommended for account naming.',
      dataImportParseEmptyObject: 'empty object',
      dataImportFieldObject: 'valid JSON object',
      dataImportFieldCredentials: 'credentials / access_token',
      dataImportFieldName: 'name / email',
      dataImportFailed: 'Data import failed',
      dataImportTimeout: 'Request timed out after {minutes} minute(s). Large archives or a slow database may need longer; try uploading smaller zip files one at a time.',
      dataImportNetworkFailed: 'Network connection failed: {detail}. Ensure the server is running, each archive is under 256MB, and avoid uploading multiple large files at once.',
      dataImportHttpFailed: 'Server returned HTTP {status}. Check backend logs for details.',
      dataImportResult: 'Import Result',
      dataImportResultSummary: 'Submitted {proxies_received} proxy record(s) and {accounts_received} account record(s). Proxies: created {proxy_created}, reused {proxy_reused}, skipped {proxy_skipped}, failed {proxy_failed}. Accounts: created {account_created}, updated {account_updated}, skipped {account_skipped}, failed {account_failed}.',
      dataImportResultSuccessTitle: 'Import completed',
      dataImportResultPartial: 'Import partially succeeded',
      dataImportResultFailed: 'Import failed',
      dataImportResultEmpty: 'No import outcome',
      dataImportNoOutcomeHint:
        'Nothing was created, updated, or failed. Ensure JSON is a sub2api backup (with accounts array) or a Codex session / openai-free single-account object (with access_token or credentials).',
      dataImportActionCreated: 'Created',
      dataImportActionUpdated: 'Updated',
      dataImportActionReused: 'Reused',
      dataImportActionSkipped: 'Skipped',
      dataImportActionFailed: 'Failed',
      dataImportKindProxy: 'Proxy',
      dataImportKindAccount: 'Account',
      ...
      columns: {
        ...
        weight: 'Weight',
        status: 'Status',
        rateLimitResume: 'Resume In',
        schedulable: 'Schedulable',
        todayStats: 'Today Stats',
        groups: 'Groups',
        usage5h: '5h Usage',
        usage7d: '7d/30d Usage',
      ...
      bulkActions: {
        ...
        refreshToken: 'Refresh Token',
        testConnection: 'Test Connection',
        testConnectionFiltered: 'Test Filtered',
        testingConnection: 'Testing...',
        testConnectionConfirm: 'Run concurrent connectivity tests for {count} selected account(s) and auto-update status from upstream responses. Continue?',
        testConnectionFilteredConfirm: 'Run concurrent connectivity tests for all {count} account(s) matching current filters and auto-update status. Continue?',
        testConnectionSuccess: 'Test complete: {count} account(s) available',
        testConnectionPartial: 'Test complete: {success} available, {failed} failed ({updated} account status updated)',
        noSelection: 'Select accounts to test first',
        resetStatusSuccess: 'Successfully reset {count} account(s) status',
        refreshTokenSuccess: 'Successfully refreshed {count} account(s) token',
        partialSuccess: 'Partially completed: {success} succeeded, {failed} failed'
      },
      bulkTestAll: 'Bulk Connectivity Test',
      bulkTestAllHint: 'Test all accounts under current filters by status; the list refreshes when done',
      bulkTest: {
        title: 'Bulk Test Connection',
        scopeSelected: '{count} account(s) selected',
        scopeFiltered: 'All accounts matching current filters',
        scopeStatuses: 'Test all accounts by status',
        statusFilterLabel: 'Account statuses to test',
        statusFilterHint: 'Tests every account matching the selected statuses under current group, platform, and search filters (not limited to the current page)',
        selectAllStatuses: 'Select all statuses',
        clearStatuses: 'Clear',
        noStatusSelected: 'Select at least one account status',
        maxAccountsWarning: 'Up to {max} accounts per run; you selected {count}. Narrow the status range or filters.',
        scopeHint: 'Account status will be auto-updated from upstream responses',
        concurrencyHint: 'Up to {workers} concurrent probes per batch, with multiple batches in parallel',
        accountCount: 'Accounts to test: {count}',
        collectingAccounts: 'Collecting accounts to test…',
        running: 'Running bulk connectivity tests...',
        stopAndRefresh: 'Stop & refresh list',
        stopping: 'Stopping…',
        refreshList: 'Refresh list',
        resultStopped: 'Test stopped',
        resultStoppedSummary: 'Completed {done}/{total}; {updated} account status(es) updated and list refreshed',
        progressDetail: 'Completed {done} / {total}',
        resultSuccess: 'Bulk test complete',
        resultPartial: 'Bulk test complete (partial failures)',
        resultSummary: '{success} available, {failed} failed, {updated} account status updated',
        stats: {
          success: 'Success',
          failed: 'Failed',
