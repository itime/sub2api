...
func (s *AccountUsageService) getOpenAIUsage(ctx context.Context, account *Account, force bool) (*UsageInfo, error) {
	...
	if stats, err := s.usageLogRepo.GetAccountWindowStats(ctx, account.ID, codexWindowStatsStart(usage.FiveHour, codexStatsWindowDuration(account.Extra, "5h", 5*time.Hour), now)); err == nil {
		if usage.FiveHour == nil {
	...
	if stats, err := s.usageLogRepo.GetAccountWindowStats(ctx, account.ID, codexWindowStatsStart(usage.SevenDay, codexStatsWindowDuration(account.Extra, "7d", 7*24*time.Hour), now)); err == nil {
		if usage.SevenDay == nil {
...
func (s *AccountUsageService) addWindowStats(ctx context.Context, account *Account, usage *UsageInfo) {
	...
	if usage.SevenDay != nil {
		...
		if account.IsOpenAIOAuth() {
			sevenDayStart = codexWindowStatsStart(
				usage.SevenDay,
				codexStatsWindowDuration(account.Extra, "7d", 7*24*time.Hour),
				now,
			)
		}
		s.attachWindowStats(ctx, account.ID, usage.SevenDay, sevenDayStart)
	}
...
func codexStatsWindowDuration(extra map[string]any, window string, fallback time.Duration) time.Duration {
	if len(extra) == 0 {
		return fallback
	}
	...
	switch window {
	case "5h":
		minutesKey = "codex_5h_window_minutes"
	case "7d":
		minutesKey = "codex_7d_window_minutes"
	default:
		return fallback
	}
	if minutes := parseExtraInt(extra[minutesKey]); minutes > 0 {
		return time.Duration(minutes) * time.Minute
	}
	return fallback
}

func codexWindowStatsStart(progress *UsageProgress, window time.Duration, now time.Time) time.Time {
