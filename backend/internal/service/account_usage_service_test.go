...
func TestCodexStatsWindowDuration(t *testing.T) {
	t.Parallel()

	if got := codexStatsWindowDuration(map[string]any{"codex_7d_window_minutes": 43200}, "7d", 7*24*time.Hour); got != 30*24*time.Hour {
		t.Fatalf("7d window = %v, want 30d", got)
	}
	if got := codexStatsWindowDuration(map[string]any{"codex_5h_window_minutes": 300}, "5h", 5*time.Hour); got != 300*time.Minute {
		t.Fatalf("5h window = %v, want 300m", got)
	}
	if got := codexStatsWindowDuration(nil, "7d", 7*24*time.Hour); got != 7*24*time.Hour {
		t.Fatalf("fallback window = %v", got)
	}
}

func TestCodexWindowStatsStart_UsesActualWindowLength(t *testing.T) {
	t.Parallel()
	...
	start := codexWindowStatsStart(progress, window, now)
	want := resetAt.Add(-window)
	if !start.Equal(want) {
